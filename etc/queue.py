...
def submit(batch_size=8, overlap=30, resolution=64, learning_rate="0.001", wall_time="01:59:00", loss="DIC", partition="gpu_8"):
    ...
    command = f'sbatch -d singleton --export={DICE}BATCH_SIZE={batch_size},OVERLAP={overlap},RESOLUTION="{resolution} {resolution} {resolution}",LEARNING_RATE={learning_rate},ID="{job_name}",LOSS={loss} --job-name="{job_name}" --partition={partition}{gpu_reservation} -t {wall_time} --output={output} --error={error} submit.sh'
