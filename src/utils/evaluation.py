...
test_dir = r'C:\Users\D070310\Aneurysm_data\10023\10023'  # For example: '/data/0'
participant_dir = r'C:\Users\D070310\Aneurysm_data\10023'  # For example: '/output/teamname/0'
...
def get_images(test_image, result_image):
...
def get_center_of_mass(result_image):
    ...
    result_array = result_image.cpu().detach().numpy()

    result_array = np.transpose(result_array, axes=[2, 0, 1])
...
def get_radius(result_image):
    ...
    structure = ndimage.generate_binary_structure(rank=result_array.ndim, connectivity=result_array.ndim)
    label_array = ndimage.label(result_array, structure)

    aneurysm_binary_array = label_array[0]
    aneurysm_number = label_array[1]

    radii_arr = []
    for i in range(aneurysm_number):
        volume = np.where(aneurysm_binary_array == i + 1)[0].shape[0]
        radius = np.cbrt(0.75 * volume / np.pi)
        radii_arr.append(radius)

    return np.array(radii_arr)
